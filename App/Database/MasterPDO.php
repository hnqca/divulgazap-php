<?php
    ...
    class MasterPDO {
        ...
        public function select(bool $selectAll = false, string $columnsToSelect = "*")
        {
            $columnsToSelect = self::doCleanParam($columnsToSelect);

            $sql  = "SELECT {$columnsToSelect} FROM {$this->table} {$this->where} {$this->groupBy} {$this->orderBy} {$this->limit}";
